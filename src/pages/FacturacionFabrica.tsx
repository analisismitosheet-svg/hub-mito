...
export default function FacturacionFabrica() {
  ...
  useEffect(() => {
    ...
    // Refresca cada 60s y al volver a enfocar la pestaña, para que el verde
    // aparezca sin recargar la página.
    const id = window.setInterval(() => void cargarRecibidos(), 60_000)
    const onFocus = () => void cargarRecibidos()
  ...
  function celdaEmpleado(r: FactRegistro) {
    ...
    const cls = tdBase
    if (!inlineEdit(r, 'quien_facturo')) return <td className={cls}>{spanSub(r.quien_facturo)}</td>
    if (activo) {
      return (
        <td className={cls} onClick={(e) => e.stopPropagation()}>
          <select
  ...
  function celdaCliente(r: FactRegistro) {
    ...
    const cls = tdBase
    if (!inlineEdit(r, 'n_cliente')) return <td className={cls}>{spanSub(r.n_cliente)}</td>
    if (activo) {
      return (
        <td className={cls} onClick={(e) => e.stopPropagation()}>
          <select
            ...
            <table className="w-full table-fixed border-collapse text-[11px] leading-tight">
              ...
              <tbody className="divide-y divide-line/50 bg-surface">
                {listaPagina.map((r) => (
                  ...
                  >
                    {isAdmin && <td className="px-1 py-[2px] text-center" onClick={(e) => e.stopPropagation()} onMouseDown={(e) => e.stopPropagation()}><input type="checkbox" checked={selected.has(r.id)} onChange={() => toggleSelect(r.id)} className="h-3 w-3 rounded border-line bg-surface2 accent-brand-600" /></td>}
